...
import com.jhcompany.graduation.Connection.ConnectionStatus;

import org.eclipse.paho.android.service.MqttAndroidClient;
import org.eclipse.paho.client.mqttv3.IMqttActionListener;
import org.eclipse.paho.client.mqttv3.IMqttToken;

import static com.jhcompany.graduation.Connection.ConnectionStatus.CONNECTED;
import static com.jhcompany.graduation.Connection.ConnectionStatus.DISCONNECTED;
import static com.jhcompany.graduation.Connection.ConnectionStatus.ERROR;
...
class ActionListener implements IMqttActionListener {
  ...
  private void connect() {
    Connection c = Connections.getInstance(context).getConnection(clientHandle);
    c.changeConnectionStatus(CONNECTED);
  ...
  private void disconnect(Throwable exception) {
    Connection c = Connections.getInstance(context).getConnection(clientHandle);
    c.changeConnectionStatus(DISCONNECTED);
    c.addAction("Disconnect Failed - an error occured");
    Toast.makeText(context, "Disconnect Failed - an error occured", Toast.LENGTH_SHORT).show();
  }
  ...
  private void connect(Throwable exception) {
    Connection c = Connections.getInstance(context).getConnection(clientHandle);
    c.changeConnectionStatus(ERROR);
    c.addAction("Client failed to connect");
    Toast.makeText(context, "Client failed to connect", Toast.LENGTH_SHORT).show();
    GraduationApplication.increaseRetryCount();
  }
}